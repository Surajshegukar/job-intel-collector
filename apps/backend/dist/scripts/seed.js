...
async function seed() {
    try {
        ...
        for (const c of MOCK_COMPANIES) {
            const companyDoc = new Company_1.Company(c);
        ...
        for (const jobData of MOCK_JOBS) {
            await JobProcessingService_1.JobProcessingService.processAndSaveJob(jobData);
        }
        console.log(`Seeded ${MOCK_JOBS.length} jobs.`);
        console.log('Creating hiring posts...');
        for (const postData of MOCK_HIRING_POSTS) {
            const comp = await Company_1.Company.findOne({ name: postData.companyName });
            if (comp) {
                const postDoc = new HiringPost_1.HiringPost({
                    companyId: comp._id,
        ...
        // Log skills created
        const allSkills = await Skill_1.Skill.find().sort({ frequency: -1 });
        ...
        await ResumeTemplate_1.ResumeTemplate.deleteMany({});
        await ResumeTemplate_1.ResumeTemplate.insertMany([
            {
                name: 'ATS Standard',
                type: 'ats',
