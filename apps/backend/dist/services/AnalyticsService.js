...
class AnalyticsService {
    ...
    static async getOverviewStats() {
        ...
        const statusAggregation = await Application_1.Application.aggregate([
            {
                $group: {
                    _id: '$status',
                    count: { $sum: 1 }
    ...
    static async getTopCompanies(limit = 10) {
        const aggregation = await Job_1.Job.aggregate([
            {
                $group: {
                    _id: '$companyId',
                    jobCount: { $sum: 1 }
     ...
     */
    static async getLocationStats(limit = 10) {
        const aggregation = await Job_1.Job.aggregate([
            {
                $group: {
                    _id: {
     ...
     */
    static async getSalaryRangeStats() {
        const jobs = await Job_1.Job.find({}, 'salary');
        const bands = {
            'Not Specified': 0,
            '< $50,000': 0,
            '$50,000 - $100,000': 0,
