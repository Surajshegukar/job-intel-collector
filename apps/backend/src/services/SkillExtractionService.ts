...
interface SkillDefinition {
  name: string;
  category: string;
  keywords: string[]; // Variations for mapping, e.g. ["js", "javascript"]
}
...
export class SkillExtractionService {
   ...
   */
  static async updateSkillFrequencies(skills: string[]): Promise<void> {
   ...
   */
  static async decrementSkillFrequencies(skills: string[]): Promise<void> {
    if (!skills || skills.length === 0) return;
    ...
    await Skill.deleteMany({ frequency: { $lte: 0 } });
  }
}
